...
exports.addresses = function (req, res) {
  ...
  client.getAddressesByAccount(`dnrw(${username})`, function (err, addresses, resHeaders) {
      ...
      res.render('account/addresses', { title: 'Mi Direccion', user: req.user, addy: addy, addresses: addresses, privkey: privkey });
...
exports.wallet = function (req, res) {
    ...
    client.getBalance(`dnrw(${username})`, 10, function (error, balance, resHeaders) {
        ...
        client.getAddressesByAccount(`dnrw(${username})`, function (err, addresses, resHeaders) {
            ...
            res.render('account/wallet', { title: 'Mi Wallet', user: req.user, usd: usdprice.toFixed(2), btc: btcprice.toFixed(8), address: address, qrcode: qrcode, balance: balance.toFixed(8), transactions: transactions });
...
exports.address = function (req, res) {
    ...
    client.getNewAddress(`dnrw(${username})`, function (error, address, resHeaders) {
        ...
        res.render('account/newaddress', { title: 'Nueva WCC Direccion', user: req.user, address: address, data_url: data_url });
    });
  });
};
...
exports.withdraw = (req, res, next) => {
    ...
    if (parseFloat(amount) - fee > balance) {
        ...
        req.flash('errors', { msg: 'Retirar mis WCC de mi Balance'});
    ...
    if (valid) {
        ...
        client.sendFrom(`dnrw(${username})`, `${sendtoaddress}`, parseFloat(`${amount}`), 10, function (error, sendFromtx, resHeaders) {
            if (error) {
                ...
                req.flash('errors', { msg: 'Fondos Insuficientes o Balance invalido!' });
                return res.redirect('/withdraw');
        ...
        req.flash('errors', { msg: 'Coloca una Wallet WCC (Clover) Valida!' });
        return res.redirect('/withdraw');
    }
  }
  });
...
exports.transactions = function (req, res) {
      ...
      client.listTransactions(`dnrw(${username})`, 10000, function (err, transactions, resHeaders) {
        ...
        res.render('account/tx', { title: 'Transacciones', user: req.user, transactions: transactions });
        });
